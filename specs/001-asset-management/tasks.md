...
- [X] T049 [P] [US1] Implement POST /auth/signup endpoint in apps/bff/src/routes/auth.ts (email/password signup with locale auto-detection)
- [X] T050 [P] [US1] Implement POST /auth/signin endpoint in apps/bff/src/routes/auth.ts (email/password signin, return JWT)
- [X] T051 [P] [US1] Implement GET /auth/me endpoint in apps/bff/src/routes/auth.ts (return current user profile)
- [X] T052 [US1] Implement GET /brokers endpoint in apps/bff/src/routes/brokers.ts (list supported brokers from seed data)
- [X] T053 [US1] Implement POST /brokers/{brokerId}/connect endpoint in apps/bff/src/routes/brokers.ts (create authorization task, issue 5-minute delegation token)
- [X] T054 [US1] Implement GET /brokers/connections endpoint in apps/bff/src/routes/brokers.ts (list user's broker connections)
- [X] T055 [US1] Implement DELETE /brokers/connections/{connectionId} endpoint in apps/bff/src/routes/brokers.ts (revoke connection)
- [X] T056 [US1] Implement POST /brokers/connections/{connectionId}/refresh endpoint in apps/bff/src/routes/brokers.ts (create collection task, call Engine)
- [X] T057 [US1] Implement GET /tasks/{taskId} endpoint in apps/bff/src/routes/tasks.ts (return task status for polling)
- [X] T058 [US1] Implement GET /portfolio endpoint in apps/bff/src/routes/portfolio.ts (return portfolio summary with total value, returns)
- [X] T059 [US1] Implement GET /portfolio/holdings endpoint in apps/bff/src/routes/portfolio.ts (list holdings with original currency values)
- [X] T060 [P] [US1] Create authorization service in apps/bff/src/services/authorization.service.ts (task creation, Engine API calls)
- [X] T061 [P] [US1] Create collection service in apps/bff/src/services/collection.service.ts (task creation, Engine API calls)
- [X] T062 [P] [US1] Create portfolio service in apps/bff/src/services/portfolio.service.ts (portfolio aggregation, holdings queries)

### Engine Implementation for User Story 1

- [X] T063 [US1] Create authorization state machine in apps/engine/src/tasks/authorization.machine.ts using xstate (pending → in_progress → paused/completed/failed states)
- [X] T064 [US1] Create collection state machine in apps/engine/src/tasks/collection.machine.ts using xstate (pending → in_progress → completed/partial/failed states)
- [X] T065 [US1] Implement POST /authorize endpoint in apps/engine/src/api/routes/authorization.ts (execute authorization state machine, handle human-in-the-loop)
- [X] T066 [US1] Implement POST /authorize/{taskId}/resume endpoint in apps/engine/src/api/routes/authorization.ts (resume paused authorization after user verification)
- [X] T067 [US1] Implement POST /collect endpoint in apps/engine/src/api/routes/collection.ts (execute collection state machine)
- [X] T068 [P] [US1] Create example broker adapter in apps/engine/src/brokers/example.adapter.ts (implements adapter interface, demonstrates Playwright automation)
- [X] T069 [P] [US1] Create Futu broker adapter in apps/engine/src/brokers/futu.adapter.ts (real broker implementation with captcha support)
- [X] T070 [US1] Implement broker adapter factory in apps/engine/src/brokers/adapter.factory.ts (select adapter by broker_id)
- [X] T071 [US1] Create authorization orchestration service in apps/engine/src/services/authorization-orchestrator.ts (state machine execution, adapter coordination)
- [X] T072 [US1] Create collection orchestration service in apps/engine/src/services/collection-orchestrator.ts (state machine execution, holdings persistence via BFF callback or direct DB write)
